...
const zieh = await seite.evaluate(() => {
  ...
  /* ⚠ BENANNTE ABWEICHUNG ZU DEN SCHWESTERN: hier blendet die Leiste
     Kategorien OHNE Inhalt aus (`if(cnt===0&&c.id!=='all')return '';`). Eine
     frisch angelegte waere dort also so oder so nicht zu sehen, und der
     Waechter haette nichts gemessen. Sie bekommt deshalb ein Rezept. */
  const fremdRein = katAnlegen('Ganz-Neu-Sortier');
  R.push({ id:8801, name:"Sortier-Neuling", cat:fremdRein, shut:true, ings:[], steps:[] });
  renderCatNav();
  const neueDa = leiste().includes(fremdRein);
  R = R.filter(r => r.id !== 8801);
  ...
  const ergebnis = {
    ...
    ordGespeichert: (localStorage.getItem('mrzcatord9m')||''),
    ordAndereApp: (localStorage.getItem('mrzcatord9')||''),
...
/* ⚠ BENANNTE ABWEICHUNG ZU DEN SCHWESTERN: hier zeigt die Leiste nur
   Kategorien MIT INHALT (`if(cnt===0&&c.id!=='all')return '';`), der Baum
   dagegen alle. Verglichen wird deshalb die SCHNITTMENGE in beide Richtungen —
   ein Vergleich der ganzen Listen waere hier immer rot, ohne dass eine
   Zusicherung gefallen waere. */
ok("der Ordner-Baum zeigt dieselbe Folge wie die Leiste",
   zieh.baumVor.filter(k => zieh.nachVor.includes(k)).join("|")
     === zieh.nachVor.filter(k => zieh.baumVor.includes(k)).join("|")
   && zieh.baumVor.filter(k => zieh.nachVor.includes(k)).length > 1);
...
ok("… unter dem eigenen Schluessel, der von Muttis Rezeptbuch bleibt leer",
   zieh.ordAndereApp === "");
...
const nachLaden = await seite.evaluate(async () => {
  ...
  const roh = localStorage.getItem('mrzcatord9m');
