...
const LANG_KEY  = "mlang9m";
const APP_NAME  = "Mein Rezeptbuch";
...
const NEUE = ["sMistralLbl","sMistralSub","sNetzHead","sNetzLbl","sNetzSub",
              "sToolsHead","sToolsSuchLbl","sToolsSuchSub","sToolsPinLbl","sToolsPinSub","sOfflineCap"];
...
const fremd = await seite.evaluate(()=>[...Array(localStorage.length).keys()]
  .map(i=>localStorage.key(i)).filter(k=>/^(mlang9|mxlang9m)$/.test(k)));
