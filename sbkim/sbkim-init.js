...
(function () {
  ...
  // Alle Gerätenamen-Felder der Seite gleichziehen. Bewusst NUR die mit der Marke:
  // das Buch-Namensfeld daneben speichert etwas anderes und darf nicht mitgezogen
  // werden. Programmatisches Setzen von .value löst kein "input" aus — keine Schleife.
  ...
  function baueNamensfeld(id, beschriftung, platzhalter, wert, speichern) {
    ...
    // Marke NUR für den Gerätenamen (INTERFACES §11.7) — das Buch-Namensfeld
    // benutzt denselben Bau-Helfer, speichert aber etwas anderes.
  ...
  function mountRendezvous() {
    ...
    if (window.SbkimRendezvous && typeof window.SbkimRendezvous.init === "function") {
      try {
        window.SbkimRendezvous.init({
          nodeName: displayNodeName("Mein Rezeptbuch"),
          dbSuffix: "rezeptbuch",
    ...
    try {
      window.addEventListener("sbkim:geraetename-changed", function () {
        ...
        try { if (window.SbkimRendezvous && window.SbkimRendezvous.configure) window.SbkimRendezvous.configure({ nodeName: displayNodeName("Mein Rezeptbuch") }); } catch (_e) {}
      });
    } catch (_e) {}
  }
